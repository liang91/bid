...
class JobManager:
    ...
    def _wrap(job_name: str, func, *args, **kwargs):
        """包装任务函数，记录执行日志."""
        id = JobLogDao.create(JobLogDto(job_name=job_name, trigger_time=datetime.now(), status=0))
        logger.info(f"[Scheduler] 任务 {job_name}:{id}开始执行")
        ...
        try:
            ...
            JobLogDao.update(id, status=1, record_count=record_count, message="success")
            logger.info(f"[Scheduler] 任务 {job_name}:{id} 执行成功")
        except Exception as e:
            JobLogDao.update(id, status=2, message=str(e))
            logger.error(f"[Scheduler] 任务 {job_name}:{id} 执行失败: {e}")
